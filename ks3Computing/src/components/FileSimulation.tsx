...
const FileSimulation = ({ onExit }: { onExit: () => void }) => {
    ...
    const checkStepCompletion = useCallback((updatedItems: Record<string, FsItem>, pathId: string, currentHistory: string[]) => {
        if (!currentStep || isStepComplete || isLevelComplete || isGameComplete) return;
        ...
        if (currentStep.isCompleted(updatedItems, pathId, currentHistory)) {
            ...
            if (isLastStep) {
                ...
                setFeedback({ message: `Level ${currentLevel.id} Complete!`, type: 'success' });
    ...
    }, [currentStep, isStepComplete, isLevelComplete, isGameComplete, stepIndex, currentLevel, levelIndex]);
 ...
 handleContainerClick = (e: React.MouseEvent) => {
    ...
    return (
        ...
        <div className="w-full font-sans select-none">
            <div className="bg-black/50 text-white p-3 rounded-t-lg text-center relative">
                 <h3 className="text-xl font-bold">
                    {isGameComplete ? "Congratulations!" : 
                     isLevelComplete ? `Level ${currentLevel.id} Complete!` : 
                     currentLevel.title}
                </h3>
        ...
        <div className="text-center p-2 rounded-b-lg h-20 flex items-center justify-center gap-4">
            {feedback && <div className={`p-4 rounded-lg shadow-xl text-white ${feedback.type === 'success' ? 'bg-green-600' : 'bg-red-600'} z-50`}>{feedback.message}</div>}
