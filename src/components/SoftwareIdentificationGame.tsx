...
const SoftwareIdentificationGame = () => {
	...
	const choiceRenderer = (
		...
		<button
			...
			onClick={onSelect}
			className="flex items-start gap-3 p-4 text-left transition-all duration-200 rounded-lg shadow-sm cursor-pointer bg-slate-100 text-slate-800 hover:bg-slate-200 hover:shadow-md focus:outline-none focus:ring-2 focus:ring-blue-500"
			type="button"
		>
			{index !== undefined && (
			...
			<ScenarioBasedGame<SoftwareChoice, SoftwareSummary>
				...
				onGameComplete={(results) =>
					logEvent({
						site: SITE,
						game: "software-id",
						score: results.summary.correctCount,
						score_max: results.summary.totalQuestions,
						correct:
							results.summary.correctCount === results.summary.totalQuestions,
					})
				}
