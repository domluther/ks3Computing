...
const TraceStage: React.FC<TraceStageProps> = ({ onComplete, onRestart }) => {
  ...
  const [message, setMessage] = useState('Move your mouse to the START circle to begin!');
  ...
  const draw = useCallback(() => {
    ...
    ctx.lineWidth = lineThickness === 'thick' ? 100 : lineThickness === 'default' ? 50 : 20;
    ...
    switch (level) {
      ...
      case 'circle': {
        ...
        startPos = { x: circleCentreX + radius * Math.cos(circleStartAngle), y: circleCentreY + radius * Math.sin(circleStartAngle) };
      ...
      case 'star':{
        ...
        const starStartAngle = (startPointIndex * 2 * Math.PI / numPoints) - Math.PI / 2;
        startPos = { 
        ...
        const starEndAngle = (endPointIndex * 2 * Math.PI / numPoints) - Math.PI / 2;
  ...
  const isPointNear = (x1: number, y1: number, x2: number, y2: number, radius = 25) => {
  ...
  return (
    <div className="w-full h-[60vh] flex flex-col items-center justify-center p-4">
      <h3 className="text-2xl font-bold text-slate-700 mb-2">Stage 1: Mouse Tracing</h3>
      ...
      <div className="mb-2">
        <label className="font-medium text-slate-600 mr-2">Line Thickness:</label>
        <select
          value={lineThickness}
      ...
      <div className="w-full h-full border-4 border-slate-300 rounded-lg overflow-hidden">
        <canvas ref={canvasRef} onMouseMove={handleMouseMove} className="w-full h-full bg-slate-50" />
      </div>
