...
const ClickStage: React.FC<ClickStageProps> = ({ onComplete, onRestart }) => {
  ...
  const handleClick = (e: React.MouseEvent, item: typeof items[number]) => {
    ...
    const isValidClick =
      (item.type === 'L' && isLeftClick) ||
      (item.type === 'R' && isRightClick);
    ...
    if ((isValidClick || doubleClickRequirement) && newClickCounts[item.id] >= requiredCount) {
  ...
  return (
    <div className="w-full h-[60vh] flex flex-col items-center p-4">
      <h3 className="text-2xl font-bold text-slate-700 mb-2">Stage 2: Clicking</h3>
      <p className="text-lg text-slate-500 mb-2 bg-yellow-100 p-2 rounded-lg">
        ...
        <span className="font-bold text-red-600"> RIGHT-CLICK</span> R or RR. Double-click LL/RR!
      </p>
      <div className="mb-2 text-lg font-semibold text-slate-700">Time: {elapsed.toFixed(1)}s</div>
      <div className="w-full h-full border-4 border-slate-300 rounded-lg relative bg-slate-50" onContextMenu={e => e.preventDefault()}>
        {items.map(item => (
          <div
            key={item.id}
